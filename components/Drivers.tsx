...
export const Drivers: React.FC = () => {
  ...
  const handleDelete = async (id: number) => { if (window.confirm('Supprimer ?')) await DataService.deleteDriver(id); loadData(); };
  const handleSubmit = async (e: React.FormEvent) => { e.preventDefault(); if (currentDriver.driver_id) await DataService.updateDriver(currentDriver.driver_id, currentDriver); else await DataService.addDriver(currentDriver as any); setIsModalOpen(false); loadData(); };
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 h-[calc(100vh-8rem)]">
      <div className="flex flex-col h-full">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-bold text-success flex items-center gap-2">Active Assignments</h2>
      ...
      <div className="flex flex-col h-full">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-bold opacity-70 flex items-center gap-2">Driver Pool</h2>
